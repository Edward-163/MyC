...
/***
   ...
   基础知识 数组作为函数参数会退化为指针
   解析A int* arr[3],[]优先级为1,很高,所以该写法是"指针数组",该数组有3个元素,元素类型是指针
        如果把int hyj[]理解为形参名hyj,类型是int[],卧槽这不就是java数组的写法么...那么int* arr[3]就可以理解为,形参名arr,类型是int*[],又因为数组作为函数参数会退化,所以int** arr
   解析B 把int (*arr)[3]理解为int hyj[3],得出*arr==hyj, 已知声明了指针变量*arr,hyj是数组,则arr指向hyj数组(含有3个int), 所以arr是数组指针,
        形参名arr,类型int (*arr)[3],这个没有解释
        int arr[2][3]理解为int[3] arr[2]:arr是长度为2的数组,数组的元素type为int[3]
   ...
   解析C int ** array 形参名array,类型int**,通常理解为指针的指针(少数情况:如leet中int** returnColSize就应该理解为一级指针的地址),没啥说的
        c不行的更深层原因有待补充,
   解析D 把int arr[][3]理解为int hyj[3],得出arr[]==hyj,
        形参名arr,类型int (*arr)[3],这个没有解释,我暂且这么理解,int arr[][3]==int[3] arr[]==又因为arr[]作为函数参数会退化==int[3] *arr
    二级指针                    二维数组(元素是连续存放的,此arr就存2行!,最开始我不理解,一度以为下面画错了,后来看了上面的链接悟了)
    pp->p->int变量            arr->1==arr[0] 2 3
                                  4==arr[1] 5 6

   int* arr[3]==int** arr     int (*arr)[3]     左侧有两次指向,右侧只有一次指向,这区别够明显吧,思考一个问题,二维数组转int**的过程,
   arr->arr[0]->int变量        arr->1 2 3
        arr[1]->int变量             4 5 6
        arr[2]->int变量
错误反思:之前粗暴把int arr[][]==int** arr,错误推出arr[row]的类型是int*,就觉得arr[row]是一级指针,arr[row]就应该指向某个实际的数组
