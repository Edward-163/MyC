...
/*
 给定一个整数数组arr，以及三个整数A，B，C，如果从数组中取出三元组（arr[i],arr[j],arr[k])
满足下列全部条件，则认为他是平衡三元组
...
统计并返回其中平衡三元组的数量
例子：
输入：  arr = [4,9,9,8,9,5,3,7]
        numA = 1
        numB = 3
        numC = 0
输出：3
        解释：一共有 3 个平衡三元组：[(9,9,9), (9,8,9), (9,8,9)] 。
        注意：其中两个 (9,8,9) 的第一个 9 在数组 arr 中的下标不同，视为 2 个平衡三元组。
 */
        ...
        for (int b = a + 1; b < size - 1; b++) {
            ...
            for (int c = b + 1; c < size; c++) {
                ...
                if (abs(arr[a], arr[c]) <= num[2]) {
                    ans++;
                    // printf("%d_%d_%d \n",arr[a],arr[b],arr[c]);fflush(stdout);
                }
            }
        }
    }
